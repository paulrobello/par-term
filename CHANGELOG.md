...
- **Update downloads are signature-verified, and every redirect hop is revalidated.** The binary and its `.sha256` came from the same release assets array via the same download function, so anyone able to replace the asset could replace the checksum — the checksum defended against corruption, not compromise. Signature verification against a compile-time-pinned key now runs before anything touches the live bundle, and fails closed. Redirects are revalidated at each hop rather than trusted after the first, on both the update and shader-install paths, and the URL scheme allowlist rejects `ftp:`, `data:` and single-slash `file:` URLs that all previously passed. **This ships a placeholder public key that is still empty**, and a test asserts that it is: self-update therefore refuses every release until a real key is pinned. That is not a regression — self-update was already inert, because no release published per-binary checksums for the existing gate.
