...
- **Pane content flash during heavy output** — when an animated custom shader triggers continuous 60fps redraws and the PTY reader holds the terminal write lock (common during heavy output from tools like Claude Code in tmux), the pane rendering path returned empty cells instead of falling back to cached content. This caused brief per-frame flashes where terminal text disappeared and only the shader background was visible. Fixed by adding a per-pane cell cache in the pane rendering path that preserves the last successfully gathered cells, mirroring the existing cache fallback in the tab-level cell extraction. Also fixed the lock-failure fallback in `extract_tab_cells` which hardcoded `is_alt_screen: false`, causing prettifier state thrashing between frames.
