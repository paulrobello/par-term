...
- **par-mux client** — a par-term tab or profile can attach to a [par-mux](https://github.com/paulrobello/par-mux) session: the profile editor has a separate **par-mux** section with `mux_session_name` and auto-attach settings independent of tmux, a fresh attach creates the session when missing, sessions survive detach and reconnect, and attach seeds the pane with the session's scrollback before live output. Behind the `mux` feature flag (currently built only through `make with-local-core` while the core library's mux feature is unpublished). See [MUX.md](docs/features/MUX.md).
