...
- **Custom-action prefix mode** — custom actions can now be triggered with a tmux-style two-stroke binding: configure a global `custom_action_prefix_key`, assign a single-character `prefix_char` to each action, then press the prefix key followed by that character. The settings UI exposes both fields and warns about duplicate prefix chars and prefix-key conflicts.
...
[Unreleased]: https://github.com/paulrobello/par-term/compare/v0.25.0...HEAD
