...
- **Missing Symbol Glyphs from Emoji Fonts (Dingbats, Misc Symbols)**: Fixed characters like ✔ ❄ ⭐ ✨ ⏩ ☑ rendering as invisible when the configured font range or fallback chain resolves to Apple Color Emoji — the font has charmap entries for these symbols but produces empty outlines (all-zero alpha) because glyph data is stored as sbix bitmaps only; the rasterizer now detects degenerate outlines and retries with color bitmap sources, converting the result to a monochrome alpha mask via luminance; if the font truly cannot render the glyph, the renderer falls back to the next font in the chain via a new `find_glyph_excluding()` method
