...
- **URL underline starts at left edge instead of URL position** — when the scrollbar was visible, URL detection used the renderer's grid column count (which excludes the scrollbar inset) while cells were laid out with the pane's narrower column count, causing row boundary misalignment and incorrect underline positions. Fix: derive actual column count from the cell array.
