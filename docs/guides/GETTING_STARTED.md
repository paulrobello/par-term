...
- **Deep customization** -- 61 background shaders, 12 cursor effects, configurable keybindings, snippets, and automation triggers
...
par-term includes 73 ready-to-use GLSL shaders: 61 background shaders (starfield, matrix rain, CRT effects, plasma, and more) and 12 cursor effect shaders (glow, trail, ripple). These are optional visual enhancements.
...
- **[Copy Mode](../features/COPY_MODE.md)** -- Vi-style keyboard-driven text selection with search, marks, and visual modes
- **[Snippets & Actions](../features/SNIPPETS.md)** -- Save text blocks with variables; create custom actions (shell commands, key sequences, splits) bound to keybindings; compose multi-step workflows with Sequence, Condition, and Repeat action types
- **[SSH Quick Connect](../features/SSH.md)** -- Browse and connect to SSH hosts from your config, known hosts, and shell history
- **[tmux Integration](../features/TABS.md)** -- Native tmux status bar rendering, bidirectional pane resize, and session picker
- **[Assistant Panel](../ASSISTANT_PANEL.md)** -- Chat with AI agents (Claude Code, GitHub Copilot, and others) directly in your terminal
- **[Custom Shaders](../features/CUSTOM_SHADERS.md)** -- Create your own GLSL shaders or port effects from Shadertoy
- **[Triggers & Automation](../features/AUTOMATION.md)** -- Regex triggers on terminal output with 8 action types and coprocess support
- **[Status Bar](../features/STATUS_BAR.md)** -- Configurable widget bar with clock, git branch, CPU/memory, and more
...
- [Integrations](../features/INTEGRATIONS.md) - Shell integration and shader installation details
- [Custom Shaders](../features/CUSTOM_SHADERS.md) - Shader configuration, uniforms, and creation guide
- [Window Management](../features/WINDOW_MANAGEMENT.md) - Window types, multi-monitor, and transparency
