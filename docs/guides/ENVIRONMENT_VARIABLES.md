...
| `RUST_LOG` | unset | Controls verbosity of the standard `log` crate. Parsed as a single case-insensitive token: `off`, `error`, `warn`, `info`, `debug`, or `trace`. Anything else (including module-level filters such as `par_term=debug`) falls back to `info`. When set, log output is also mirrored to stderr. A `log_level` setting in the config file overrides `RUST_LOG`; the `--log-level` CLI flag (used by `make run-debug`/`make run-trace`) overrides both. |
