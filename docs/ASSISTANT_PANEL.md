...
- **Debug file paths and diagnostics**: Resolved locations of the transpiled WGSL output (`par_term_<name>_shader.wgsl`) and wrapped GLSL (`par_term_debug_wrapped.glsl`), both in the system temp directory — `$TMPDIR` on macOS, `/tmp` on Linux, `%TEMP%` on Windows — plus guidance to call the `shader_diagnostics` MCP tool for live active shader state and last compile/reload errors. Both dumps are written only by debug builds, and `dev-release` is not one — the reported path may therefore name a file that does not exist
- **Available uniforms**: Shadertoy-compatible uniforms (`iTime`, `iResolution`, `iMouse`, `iChannel0`..`iChannel3` user texture channels), par-term-specific terminal content in `iChannel4`, terminal-aware extras (`iProgress`, `iCommand`, `iFocusedPane`, `iScroll`), and cursor-specific extras (`iCurrentCursor`, `iPreviousCursor`, `iTimeCursorChange`)
