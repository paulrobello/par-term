...
- **Available uniforms**: All Shadertoy-compatible uniforms (`iTime`, `iResolution`, `iMouse`, `iChannel0-4`) plus terminal-aware extras (`iProgress`, `iCommand`, `iFocusedPane`, `iScroll`) and cursor-specific extras (`iCurrentCursor`, `iPreviousCursor`, `iTimeCursorChange`)
