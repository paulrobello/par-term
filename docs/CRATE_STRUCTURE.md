...
| `par-term-prettifier` | Content prettifier framework — detects structured content in terminal output (Markdown, JSON, YAML, diffs, stack traces, diagrams) and renders it with rich syntax highlighting and inline graphics. Pluggable detector/renderer architecture; ships 8 built-in format renderers including native Mermaid diagram support. |
...
| `par-term-settings-ui` | All 28 settings tab modules (appearance, window, terminal, input, effects, notifications, AI inspector, integrations, advanced, etc.), the sidebar navigation component, and section search. Built on egui; depends only on `par-term-config`. |
