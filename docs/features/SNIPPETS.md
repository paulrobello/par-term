...
This works alongside the existing per-action keybinding, so an action can have either trigger style or both. If `prefix_char` is unset and a global prefix key is set, a single-character `keybinding` is the prefix follow-up only — it is not also a global shortcut.
