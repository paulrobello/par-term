...
*   **Menu (`src/menu/`)**: Native menu bar using `muda` — a macOS global menu and a Windows per-window menu. The same menu is built on Linux but never attached: `muda` needs a `gtk::Window` and winit's X11/Wayland backends do not create one, so Linux users reach these actions through keybindings. See `src/menu/linux.rs`.
