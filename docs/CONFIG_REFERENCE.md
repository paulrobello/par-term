...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `log_level` | `enum` | `warn` | Debug log verbosity: `off`, `error`, `warn`, `info`, `debug`, `trace`. Overrides the RUST_LOG environment variable; overridden by the --log-level CLI flag |
