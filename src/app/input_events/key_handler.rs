...
//!
//! This file handles all keyboard input routing:
//! - `handle_key_event`: main key dispatch entry point
//! - Per-key-category helpers: scroll, config reload, clipboard history,
//!   command history, paste, search, AI inspector, utility shortcuts,
//!   tab shortcuts, profile shortcuts
//! - Profile shortcut matching and shortcut string building
...
impl WindowState {
    ...
    fn handle_ai_inspector_toggle(&mut self, event: &KeyEvent) -> bool {
        if event.state != ElementState::Pressed {
            return false;
        }
