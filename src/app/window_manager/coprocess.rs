...
impl WindowManager {
    ...
    pub fn start_coprocess(&mut self, config_index: usize) {
        ...
        {
            if config_index >= ws.config.load().automation.coprocesses.len() {
                log::warn!("Coprocess config index {} out of range", config_index);
                return;
            }
            let coproc_config = &ws.config.load().automation.coprocesses[config_index];
        ...
        let (running_state, error_state, new_output): (Vec<bool>, Vec<String>, Vec<Vec<String>>) =
                ...
                && let Some(tab) = ws.tab_manager.active_tab()
            {
                if let Ok(term) = tab.terminal.try_read() {
                    ...
                    for (i, _) in ws.config.load().automation.coprocesses.iter().enumerate() {
                        let has_id = tab
                            .scripting
                            .coprocess_ids
                            .get(i)
