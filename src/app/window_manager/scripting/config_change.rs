        ...
        match key {
            ...
            "window_opacity" => {
                if let Some(v) = value.as_f64() {
                    ...
                    // color when the window is non-opaque; re-assert an opaque
                    // titlebar background regardless of content opacity.
                    #[cfg(target_os = "macos")]
                    if let Some(window) = &ws.window
                        && let Err(e) = crate::macos_metal::restore_opaque_titlebar(window)
                    {
