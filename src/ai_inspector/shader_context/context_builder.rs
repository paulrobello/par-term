...
pub fn build_shader_context(config: &Config) -> String {
    ...
    ctx.push_str("// control color alpha=true label=\"Overlay\"\n");
    ...
    ctx.push_str("- Color controls attach to `uniform vec3` (RGB picker) or `uniform vec4`. Use `alpha=true` only with `vec4` for an RGBA picker; omit it or set `alpha=false` for RGB. `label=\"Name\"` customizes the Settings label.\n");
