...
fn ensure_titlebar_backing_bar(ns_window: &objc2_app_kit::NSWindow) -> Result<()> {
    ...
    if let Some(bar) = find_titlebar_backing_bar(&frame_view) {
        bar.setHidden(false);
        // Refresh the fill so light/dark appearance switches are picked up.
    ...
    // Titlebar thickness: contentLayoutRect (window coords, bottom-left
    // origin) excludes the titlebar, so its minY is where the bar starts.
    let layout_rect = ns_window.contentLayoutRect();
    let frame_bounds = frame_view.bounds();
    let titlebar_height = frame_bounds.size.height - layout_rect.origin.y;
    if titlebar_height <= 1.0 {
        anyhow::bail!("window has no titlebar strip to back");
    }
