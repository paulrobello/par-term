...
| `window_state/render_pipeline.rs` | 2,753 | GPU render pipeline; `submit_gpu_frame` is a 1,454L monolithic pass — no practical split points |
...
`render_pipeline.rs` at 2,753L and `mod.rs` at 1,233L remain above the 800L target. Both are accepted:
