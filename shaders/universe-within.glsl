...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    // vec2 M = iMouse.xy/iResolution.xy-.5;  // Mouse parallax disabled
    ...
    vec2 st = uv*rot;
    // M *= rot*2.;  // Mouse parallax disabled
    ...
    for(float i=0.; i<1.; i+=1./NUM_LAYERS) {
        ...
        m += fade * NetLayer(st*size, i, iTime);  // Removed M*z for mouse parallax
    }
    ...
    if (terminalBrightness < brightnessThreshold) {
        fragColor = mix(terminalColor, vec4(col, 1.0), 0.7);
    } else {
