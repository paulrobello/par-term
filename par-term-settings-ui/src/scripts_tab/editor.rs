...
pub(super) fn show_script_edit_form(
    ...
    ui.indent("script_edit_form", |ui| {
        ...
        ui.horizontal(|ui| {
            ui.label("Name:");
            ui.text_edit_singleline(&mut settings.scripts_tab.temp_script_name);
        });

        // Script path field
        ui.horizontal(|ui| {
            ui.label("Script path:");
            ui.text_edit_singleline(&mut settings.scripts_tab.temp_script_path);
            if ui.button("Browse...").clicked()
                && let Some(path) = settings.pick_file_path("Select script file")
            {
                settings.scripts_tab.temp_script_path = path;
            }
        });
        ...
        ui.horizontal(|ui| {
            ui.label("Arguments:");
            ui.text_edit_singleline(&mut settings.scripts_tab.temp_script_args);
        });

        // Subscriptions
        ui.horizontal(|ui| {
            ui.label("Subscriptions:");
            ui.text_edit_singleline(&mut settings.scripts_tab.temp_script_subscriptions)
                .on_hover_text(
        ...
        // Options
        ui.checkbox(&mut settings.scripts_tab.temp_script_enabled, "Enabled")
            .on_hover_text("Whether this script is active");
        ui.checkbox(
            &mut settings.scripts_tab.temp_script_auto_start,
        ...
        ui.horizontal(|ui| {
            ...
            .selected_text(settings.scripts_tab.temp_script_restart_policy.display_name())
            .show_ui(ui, |ui| {
                for &policy in RestartPolicy::all() {
                    ui.selectable_value(
                        &mut settings.scripts_tab.temp_script_restart_policy,
                        policy,
                        policy.display_name(),
                    );
                }
...
fn show_permissions_section(ui: &mut egui::Ui, settings: &mut SettingsUI) {
    ...
    ui.indent("script_permissions", |ui| {
        ui.checkbox(
            &mut settings.scripts_tab.temp_script_allow_write_text,
            "Allow WriteText",
        )
        .on_hover_text(
        ...
        if settings.scripts_tab.temp_script_allow_write_text {
            ui.indent("write_text_rate", |ui| {
                ui.checkbox(
                    &mut settings.scripts_tab.temp_script_prompt_before_write_text,
        ...
        ui.checkbox(
            &mut settings.scripts_tab.temp_script_allow_run_command,
            "Allow RunCommand",
        )
        .on_hover_text(
        ...
        ui.checkbox(
            &mut settings.scripts_tab.temp_script_allow_change_config,
            "Allow ChangeConfig",
        )
        .on_hover_text(
...
fn show_save_cancel(
    ...
    ui.horizontal(|ui| {
        let can_save = !settings.scripts_tab.temp_script_name.trim().is_empty()
            && !settings.scripts_tab.temp_script_path.trim().is_empty();
            ...
            .clicked()
        {
            ...
            let new_script = ScriptConfig {
                name: settings.scripts_tab.temp_script_name.trim().to_string(),
                enabled: settings.scripts_tab.temp_script_enabled,
                script_path: settings.scripts_tab.temp_script_path.trim().to_string(),
                args,
                auto_start: settings.scripts_tab.temp_script_auto_start,
                restart_policy: settings.scripts_tab.temp_script_restart_policy,
                ...
                allow_write_text: settings.scripts_tab.temp_script_allow_write_text,
                prompt_before_write_text: settings.scripts_tab.temp_script_prompt_before_write_text,
                allow_run_command: settings.scripts_tab.temp_script_allow_run_command,
                allow_change_config: settings.scripts_tab.temp_script_allow_change_config,
                write_text_rate_limit: settings.scripts_tab.temp_script_write_text_rate_limit,
                run_command_rate_limit: settings.scripts_tab.temp_script_run_command_rate_limit,
            };
