...
fn show_clipboard_section(
    ...
    collapsing_section(
        ...
        |ui| {
            ...
            ui.horizontal(|ui| {
                ui.label("Source copy modifier:");
                if ui
                    .add(
                        egui::TextEdit::singleline(&mut clip.source_copy_modifier)
                            .desired_width(80.0),
                    )
                    .changed()
                {
            ...
            ui.horizontal(|ui| {
                ui.label("Vi copy mode:");
                egui::ComboBox::from_id_salt("prettifier_vi_copy")
                    .selected_text(clip.vi_copy_mode.as_str())
                    .show_ui(ui, |ui| {
                        if ui
                            .selectable_label(clip.vi_copy_mode == "source", "source")
                            .clicked()
                        {
                            clip.vi_copy_mode = "source".to_string();
                            settings.has_changes = true;
                            *changes_this_frame = true;
                        }
                        if ui
                            .selectable_label(clip.vi_copy_mode == "rendered", "rendered")
                            .clicked()
                        {
                            clip.vi_copy_mode = "rendered".to_string();
                            settings.has_changes = true;
                            *changes_this_frame = true;
                        }
                    });
        ...
        |ui| {
                ...
                settings.test_detection_result
            {
                ...
                if format_id.is_empty() {
                    ui.colored_label(egui::Color32::from_rgb(200, 100, 100), "No format detected.");
                } else {
