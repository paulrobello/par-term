                    ...
                    ui.group(|ui| {
                        ui.push_id(format!("custom_agent_{i}"), |ui| {
                            ...
                            ui.horizontal(|ui| {
                                ...
                                ui.label("Protocol").on_hover_text(
                                    "ACP is currently the only supported protocol.",
                                );
                                ui.add_enabled(
                                    false,
                                    egui::TextEdit::singleline(&mut agent.protocol)
                                        .desired_width(56.0),
                                )
                                .on_hover_text(
                                    "Read-only: only `acp` is supported right now.",
                                );

                                ui.label("Type").on_hover_text(
                                    "Agent category label (for organization/filtering).",
                                );
                                changed |= ui
                                    .add(
                            ...
                            let mut install_command =
                                agent.install_command.clone().unwrap_or_default();
                            if ui
                            ...
                            let mut wildcard =
                                agent.run_command.get("*").cloned().unwrap_or_default();
                            ui.horizontal(|ui| {
                                ui.label("*")
                            ...
                            let mut macos =
                                agent.run_command.get("macos").cloned().unwrap_or_default();
                            ui.horizontal(|ui| {
                                ui.label("macos")
                            ...
                            let mut linux =
                                agent.run_command.get("linux").cloned().unwrap_or_default();
                            ui.horizontal(|ui| {
                                ui.label("linux")
                            ...
                            ui.label(
                                "These key/value pairs are injected into the ACP subprocess.",
                            );
