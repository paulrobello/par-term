...
help:
	...
	@echo ""
	@echo "Grind Loop (unattended /work-loop via ~/Repos/par-grind):"
	@echo "  make grind-start        - Start the grind loop (foreground; z.ai backend, effort high)"
	@echo "  make grind-stop         - Stop a running loop after the current iteration"
	@echo "  make grind-clean-logs   - Delete all but the newest 2 grind run dirs"
	@echo "                            (backends: grind-start-anthropic/zai/grok/codex/omp)"
	@echo ""
	@echo "Other:"
...
run-debug:
	...
	@echo ""
	RUST_LOG=debug DEBUG_LEVEL=3 $(RUN_BASE) -- --log-level debug 2>&1 | tee $(DEBUG_LOG)
...
run-trace:
	...
	@echo "In another terminal, run: make tail-log"
	@echo ""
	RUST_LOG=trace DEBUG_LEVEL=4 $(RUN_BASE) -- --log-level trace 2>&1 | tee $(DEBUG_LOG)
...
# Run CMD (default: cargo check --workspace) against the local core checkout.
# Manifests auto-revert on exit; never commit the patched state.
with-local-core:
